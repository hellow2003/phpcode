...
while true; do
  ...
  for SRC in github gitlab gitea bitbucket codeberg; do
    ...
    if [[ -z "$LAST_VERSION" ]] || version_gt "$CURRENT_VERSION" "$LAST_VERSION"; then
      log_exec "SRC=$SRC NEW_VERSION=$CURRENT_VERSION → EXECUTING COMMANDS"
      ...
      log_exec "SRC=$SRC VERSION UPDATED TO $CURRENT_VERSION"
      update_found=true
    fi
  done
# thisistesting
  # Check for no updates in this full cycle
  if [ "$update_found" = false ]; then
    no_update_count=$((no_update_count + 1))
    log_exec "No update found this cycle | Consecutive: $no_update_count/$MAX_NO_UPDATE"

    if [ $no_update_count -ge $MAX_NO_UPDATE ]; then
      log_exec "3 consecutive cycles with NO UPDATE → Sleeping for 10 minutes"
      sleep 600   # 10 minutes
      no_update_count=0
      log_exec "Woke up after 10 min sleep → Resuming checks"
    fi
  else
    no_update_count=0
    log_exec "Update found this cycle → Counter reset"
  fi
