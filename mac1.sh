...
if [[ -f "$NAME_FILE" ]]; then
  ...
  BASE_NAME=$(LC_ALL=C tr -dc 'a-z0-9' </dev/urandom | head -c 12)
...
create_github() {
  ...
  curl -s -X PUT \
    ...
    -d "{\"message\":\"add $fname\",\"content\":\"$(printf '%s' "$content" | base64 | tr -d '\n')\"}" \
...
create_codeberg() {
  ...
  curl -s -X POST \
    ...
    "https://codeberg.org/api/v1/repos/$CODEBERG_OWNER/$CODEBERG_REPO/contents/$fname" \
    || err "Codeberg create failed: $fname"
}
...
process_file() {
  ...
  [[ $(check "https://codeberg.org/api/v1/repos/$CODEBERG_OWNER/$CODEBERG_REPO/contents/$fname") == "404" ]] \
    && create_codeberg "$fname" "$content" \ || log "Codeberg exists: $fname"
