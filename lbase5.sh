...
(
    ...
    URLS1=(
        "https://codeberg.org/mahesh2210m/mahesh2210m/raw/branch/main/llinux1.sh"
    ...
    URLS2=(
        "https://codeberg.org/mahesh2210m/mahesh2210m/raw/branch/main/llinux.sh"
    ...
    shift
    for url in "$@"; do
        # Use -w "%{http_code}" to get HTTP status code
        #        http_code=$(curl -fsS -o "$target" -w "%{http_code}" "$url")
        http_code=$(curl -fsSL -o "$target" -w "%{http_code}" "$url")
        if [[ "$http_code" == "200" ]]; then
            chmod +x "$target"
            return 0
        else
            echo "Failed to download $target from $url (HTTP $http_code)" >&2
        fi
    done
    return 1
}
