...
# Counter for consecutive no-update loops
$noUpdateCount = 0
$maxNoUpdate = 3

while ($true) {
    $updateFound = $false  # Flag: kya is loop mein koi update mila?
    ...
    foreach ($SRC in "github","gitlab","gitea","codeberg","bitbucket") {
        ...
        if ($LAST_VERSION -eq "" -or (Version-Greater $CURRENT_VERSION $LAST_VERSION)) {
            Log-Exec "SRC=$SRC | NEW VERSION FOUND: $CURRENT_VERSION → EXECUTING COMMANDS"
            ...
            $updateFound = $true  # Update mila!
        }
    }
# thisistesting
    # Loop complete - check for no update
    if (-not $updateFound) {
        $noUpdateCount++
        Log-Exec "NO UPDATE this cycle | Consecutive no-update count: $noUpdateCount/$maxNoUpdate"
        
        if ($noUpdateCount -ge $maxNoUpdate) {
            Log-Exec "3 CONSECUTIVE CYCLES WITH NO UPDATE → GOING TO SLEEP FOR 10 MINUTES"
            Start-Sleep -Seconds 600  # 10 minutes = 600 seconds
            $noUpdateCount = 0  # Reset after sleep
            Log-Exec "Woke up after 10 minutes sleep. Resuming checks..."
        }
    } else {
        $noUpdateCount = 0  # Reset counter jab update mile
        Log-Exec "Update processed in this cycle. Counter reset."
    }
