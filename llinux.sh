...
while true; do
  ...
  # Full cycle complete - check for updates
  if [ "$update_found" = false ]; then
    no_update_count=$((no_update_count + 1))
    log_exec "No update in this cycle | Consecutive no-update: $no_update_count/$MAX_NO_UPDATE"

    if [ $no_update_count -ge $MAX_NO_UPDATE ]; then
      log_exec "3 consecutive cycles with NO UPDATE → Sleeping for 10 minutes"
      sleep 600  # 10 minutes
      no_update_count=0
      log_exec "Awake after 10 minute sleep → Resuming checks"
    fi
  else
    no_update_count=0
    log_exec "Update found in this cycle → Counter reset"
  fi
